...
//			Arrays.fill(beta[i], 0.5 / nWords);
//		}
//		
//		LDAGibbsSampler<String> lda =  new LDAFullBetaGibbsSampler<String>(documentProvider, alpha, beta, random);
		
		LDAGibbsSampler<String> lda =  new LDAGibbsSampler<String>(documentProvider, LDAGibbsSampler.symmetricAlpha(0.1, 20), 0.1, random);
	...
	public void run(int steps, int avgC) throws IOException {
		...
		for (int i = 0; i < avgC; i++) {
			documentProviders[i] = new APParser(true, true).getCorpusDocumentProvider(new File(
...
//					0, null, 0.5, 0.8);
	...
	protected void runLDAGibbsSampler(int step, int repeat,
		...
		PwtProvider ldaFrequencyProvider = new PwtProvider() {
			@Override
...
//		System.out.println(sum + " Ratio: " + sum / sum2);
	...
	public static void main(String[] args) throws IOException {
		new TWCPerplexityErrorRateUpdateAlpha(new Random(42), 300, "").run(1,
				1);
	}
}
