...
public class TGPerplexityCalculator<T> {
	...
	public double computePerplexity(DocumentProvider<T> testDocumentProvider,
			TGSolution<T> s) {
		if (!testDocumentProvider.getVocab()
				.equals(s.getVocab())) {
	...
	public double computeLogProbability(Document<T> refD, Document<T> d,
		...
		while (it.hasNext()) {
			...
			if (wordFr > 0) {
				...
				int topicIndex = s.getTopicForWord(index);
				TIntCollection words = s.getTopics()[topicIndex];
				res += wordFr
						* computeWordLogProbability(index, refD, s, words,
								topicIndex);
			}
		}
		return res;
	...
	protected double computeWordLogProbability(int wIndex, Document<T> refD,
			TGSolution<T> s, TIntCollection words, int topicIndex) {
		// Estimate log p(w|t):
		double logpwt = Math.log(((double) s.getGlobalWordFrequency(wIndex))
				/ s.getTopicFrequency(topicIndex));
		double logptd;

		if (refD == null) {
			// No reference document available: Estimate log ptd via log pt
			...
			while (it.hasNext()) {
				topicFrInDoc += refD.getWordFrequency(it.next());
			}
			// Estimate log p(t|d)
			logptd = Math.log(smoothedPtd(topicFrInDoc, refD.getSize(), wIndex,
					topicIndex, s));
		}
