...
public abstract class AbstractMulitParamLDAGibbs<T> {
	...
	protected double[] createAlpha(double alphaConc, int topics) {
		return LDAGibbsSampler.symmetricAlpha(alphaConc, topics);
	}
	...
	protected LDAGibbsSampler<T> createSampler(double baseAlpha,
		...
		return new LDAGibbsSampler<T>(random, trainingDocumentProvider,
				createAlpha(baseAlpha, topics), createBeta(baseBeta, topics));
	}
}
